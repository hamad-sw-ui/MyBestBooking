 ...
 */
export function PriceAlertButton({ propertyId, currency = "EUR", defaultMax = 100 }: Props) {
  ...
  async function save() {
    ...
    try {
      const r = await fetch("/api/price-alerts", {
        ...
        body: JSON.stringify({ propertyId, maxPrice: n, currency }),
      });
  ...
  return (
    <div className="flex flex-col gap-2 border border-gray-200 rounded-lg p-3 bg-white">
      <p className="text-xs text-gray-600 flex items-center gap-1">
        <BellRing className="w-3 h-3" /> M&apos;alerter si le prix descend sous :
      </p>
