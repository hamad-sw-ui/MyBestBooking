...
export function ChangePasswordForm() {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div>
        <label htmlFor="cp-old" className="block text-sm font-medium text-gray-700 mb-1">Mot de passe actuel</label>
      ...
      <div>
        <label htmlFor="cp-new" className="block text-sm font-medium text-gray-700 mb-1">Nouveau mot de passe</label>
        <input id="cp-new" type="password" required minLength={8} value={newPw} onChange={(e) => setNewPw(e.target.value)} className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#1B3A6B]" placeholder="Min. 8 caractères" />
      </div>
      <div>
