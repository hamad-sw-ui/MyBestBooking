...
export function StripePaymentForm({ clientSecret, onSubmitted }: { clientSecret: string; onSubmitted: () => Promise<void> | void }) {
  ...
  useEffect(() => {
    ...
    async function loadConfiguration() {
      try {
        ...
        if (!response.ok || !body.publishableKey) throw new Error("Le paiement en ligne n’est pas configuré");
        if (active) setStripePromise(loadStripe(body.publishableKey));
      } catch (reason) {
        if (active) setError(reason instanceof Error ? reason.message : "Le paiement en ligne n’est pas configuré");
      }
    }
  ...
  if (error) return <p role="alert" className="p-3 rounded-lg bg-amber-50 text-sm text-amber-900">{error}. Aucun paiement n&apos;a été confirmé.</p>;
  if (!stripePromise) return <p className="text-sm text-gray-500"><Loader2 className="inline w-4 h-4 mr-2 animate-spin" />Chargement du paiement sécurisé…</p>;
  return (
