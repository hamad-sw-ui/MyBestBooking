...
export async function POST(request: NextRequest) {
  ...
  if (user?.role !== "admin") return NextResponse.json({ error: "Accès admin requis" }, { status: 403 });
  const rl = rateLimit(`admin:provider-rotation:${user.id}`, { limit: 3, windowMs: 60_000 });
  if (!rl.ok) return NextResponse.json({ error: "Trop de tentatives de rotation, réessayez plus tard" }, { status: 429 });
  ...
  try {
    ...
    return NextResponse.json({ error: "Impossible de réchiffrer les credentials providers" }, { status: 500 });
  }
}
