...
export async function GET(
  ...
  const [row] = await db
    ...
    .limit(1);
  if (!row?.message) return NextResponse.json({ error: await apiError("Introuvable") }, { status: 404 });
  ...
  if (!row.message.attachmentKey) {
    ...
    return NextResponse.json({ error: await apiError("Pièce jointe historique indisponible de manière sécurisée") }, { status: 410 });
  }
  const file = await (await getUploader()).get(row.message.attachmentKey);
  if (!file) return NextResponse.json({ error: await apiError("Fichier introuvable") }, { status: 404 });
