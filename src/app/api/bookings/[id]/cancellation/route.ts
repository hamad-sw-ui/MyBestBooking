...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!row) return NextResponse.json({ error: await apiError("Réservation introuvable") }, { status: 404 });
  ...
  if (row.booking.status !== "confirmed" && row.booking.status !== "pending") {
    return NextResponse.json({ error: await apiError("Cette réservation ne peut plus être annulée") }, { status: 409 });
  }
