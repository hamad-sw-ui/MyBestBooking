...
import { initialRoleFromSearchParam } from "@/lib/host-entry";
...
export default function RegisterPage() {
  ...
  const [isHost, setIsHost] = useState(() => {
    // T-180 : le lien « Ajouter mon hébergement » (footer, non-hôte) pointe
    // ici — présélectionne le rôle via `?role=host`. Sans paramètre : le
    // voyageur reste la valeur historique. Même pattern que referralCode.
    if (typeof window === "undefined") return false;
    return initialRoleFromSearchParam(new URLSearchParams(window.location.search).get("role"));
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // T-173 : invalidation du cache des préférences (cf. login) — la langue
      // choisie sur le formulaire est déjà persistée côté compte.
      invalidateDisplayPreferences();
      // T-174 : idem pour le cache favoris (figé null pour l'anonyme).
