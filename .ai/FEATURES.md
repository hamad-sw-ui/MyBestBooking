...
| **Paiement Stripe sécurisé** | 🟠 | PaymentProvider + Stripe Elements + webhook signé. Le hold DB est committé avant tout appel PSP, l’intent et les remboursements utilisent une clé d’idempotence, le cron reprend les holds; un succès tardif est compensé sans ressusciter le séjour. Clés configurables depuis coffre/env ; validation Stripe test-mode encore requise avant production. **T-152** : les réservations `pending` (intent expiré/abandon) sont actionnables — « Payer maintenant » depuis `/mes-reservations` → `/reservation?booking={id}` avec reprise automatique du paiement et gardes d'état | T-020, T-102, T-103, T-107, T-152 |
...
| **Versements hôtes/admins (ledger interne)** | ✅ | Ledger `payouts`/`payoutAccounts` (migration additive 0018) + `PayoutProvider` (mock/dev, Stripe si clés) + UI « Versements » + webhook `payout.*` idempotent. Un **payout par devise** (`createPayoutsForPeriod`, jamais de somme inter-devises), **garde-fou devise** à l'exécution (devise ≠ compte → `pending`/`skipped`), **chemin admin** (agrégation par hôte, admin jamais exécuteur d'autrui), audit `payout.*`. Tâche cron `/api/cron/payouts` (runner local + Vercel, `GET`). **Bouton par devise** (`PayoutRequestButton` envoie `currency`, POST filtre), garde-fou **visible en UI** (`skipped`/`hasAccount`), export ledger `GET /api/dashboard/billing/export-payouts` (CSV, host/admin), carte « Factures » reliée à ce ledger (pas à l'export bookings). **Externe Stripe Connect = INSPECTION** (§13.5, clés absentes) — jamais simulé payé. | T-195 |
...
| **Paiement manuel (T-202)** | ✅ | `POST /api/bookings` ne déclenche **plus** de paiement automatique → `status:"pending"`, `payment:null`, `manualConfirmation:true`. Webhook Stripe ne force plus `confirmed`. Les **états de réservation** sont gérés à la main (hôte confirme `pending→confirmed` avec `confirmedBy`). `booking-lifecycle` : `host_all` | T-202 |
| **Paiement sur place constatable (T-203)** | ✅ | Un hôte/admin peut **constater le paiement sur place** via `PUT /api/bookings/[id] {markPaidOffline:true}` → `paymentStatus:"paid"` + `paymentMethodOffline:true` (audit `booking.pay.offline`). Débloque les **versements/revenus** (pipeline `payout` filtre `paid`). Les **demandes manuelles ne sont plus expirées** après 15 min (`paymentExpiresAt:null` sans `payOnline` + cron limité aux `paymentIntentId`). UI : bouton « Marquer payé sur place » + badge « Payé sur place » ; « Payer maintenant » masqué pour les manuels | T-203 |
...
| **Garde UI Stripe — flux manuel (T-204)** | ✅ | Une réservation **manuelle** (`manualConfirmation:true`) n'affiche **jamais** l'UI de carte, même si le serveur renvoyait un `payment` malgré tout. Décision extraite dans `shouldShowStripeForm` (`src/lib/booking-flow.ts`, priorité manuel) et appliquée à `handleSubmit` + `resumePaymentFor`. La machinerie Stripe reste (back-office `/api/bookings/[id]/payment`) mais est inatteignable pour un booking manuel (reprise → 409) | T-204 |
