...
| 2026-08-27 | R10 git_branch | **Corrigée** (v3.0.1) | Le nom de branche était codé en dur (`arena/01a02dbb-…`) : toute nouvelle session Arena échouait R10 de façon bloquante. R10 est désormais pilotée par `manifest.git.branch_patterns` (RegExp `^arena/[0-9a-f]+-mybestbooking$`) + `protected_branches` (main). Vérifiée par R10 OK sur `arena/01a042cf-…`. |
| 2026-08-27 | R14 db_api_coverage | **Corrigée** (v3.0.1) | ADR-006 exige de vérifier les tables réelles de `src/db/schema.ts`. Or l'implémentation lisait uniquement `manifest.product_coverage.expected_endpoint_tables`, **absent du manifest** → la règle passait sur « 0 tables » (garde-fou vacillant). R14 extrait maintenant les `pgTable(...)` du schéma (exemptions via `exempted_tables`), et échoue explicitement si aucune table n'est trouvée. Détecte désormais 5 couvertures implicites (review_votes, upload_objects, wishlist_items, provider_credentials, audit_log). |
| 2026-08-27 | R15 ui_api_coverage | **Corrigée** (v3.0.1) | Sans `ui_action_labels` configuré, la RegExp `>\s*(|)\s*<` matchait toute paire de balises avec une capture **vide** → 29 faux positifs « bouton "" ». Labels par défaut ajoutés au manifest + capture exigeant un label réel non vide. R15 OK (0 suspect). |
| 2026-08-27 | R11 id_collision | **Corrigée** (v3.0.1) | Le simple partage d'un numéro entre séries indépendantes (BUG-nnn / T-nnn, §8.1) générait un avertissement permanent de 34 lignes qui noyait le vrai signal (B-nnn résiduels). Le partage est désormais mentionné dans le message OK ; seuls les B-nnn résiduels font échouer. |
| 2026-08-27 | R12 impact_reports_for_S_or_C | **Durcie** (v3.0.1) | La règle se contentait de l'existence d'un rapport *quelconque* ; elle vérifie maintenant que les rapports portent sur l'ID de `CURRENT_TASK.md` (T-112 → `*T[-_]?112*`). |
| 2026-08-27 | manifest v3.0.1 | **Étendu** | Ajout de `git.branch_patterns/protected_branches`, `product_coverage` (exempted_tables, ui_action_labels, seuils de fraîcheur), et de `FEATURES.md` + `PRODUCT_ACCEPTANCE.md` aux `mandatory_documents` (ils existaient déjà mais n'étaient pas déclarés, conformément à ADR-006). |
| 2026-08-27 | CI GitHub Actions | **Ajoutée** (`.github/workflows/ci.yml`) | Le rapport `REPORTS/ci_workflow_a_ajouter.md` (Session 4) attendait toujours une installation manuelle. La CI applique maintenant ADR-002 : `ai:check · lint · typecheck · test · build` sur Node 22 + Postgres 16 à chaque push/PR. |
| 2026-08-27 | Docs `.ai/` | **Réalignées** | `INDEX.md` listait des fichiers inexistants et omettait FEATURES/PRODUCT_ACCEPTANCE/UI/API ; `.ai/README.md` utilisait des commandes PowerShell et une carte des fichiers obsolète (`PROJECT_CONTEXT.md`, `libs.versions.toml` Android). Corrigés. |
| 2026-08-27 | `CODING_RULES.md` en-tête + §23 | **Corrigé** (v3.0.1) | L'en-tête déclarait tout le fichier « archive Kotlin/Android » alors que §13–§22 sont les règles AI-DOS en vigueur. En-tête clarifié (§1–§12 archive, §13–§23 opposables) et ajout d'une **§23 — Conventions de gouvernance** documentant canoniquement le nommage des branches (R10, ex-§8) et la convention d'IDs T-/BUG-/B- (R11, ex-§8.1). Les commentaires de `check-ai.mjs` pointent désormais vers §23.1/§23.2. |
| 2026-08-27 | `.gitignore` | **Étendu** | Ignore `.data/` (données du Postgres embarqué, ~65 Mo) pour ne jamais versionner la base locale. |
