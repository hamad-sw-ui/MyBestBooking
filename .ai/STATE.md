...
- **Implémentation de l'audit n°6 (2026-09-11) — lots A et B (complet)** : décisions **oui ×4**
  ...
  **3/3** (base réelle : 23 chambres → aucun bandeau ; 26 lignes → « 25 résultats affichés sur 26 » ;
  ...
  (`rate-limit.test.ts` 11/11). **T-260 → lot C (1/2)** : `/recherche` expose le tri « Populaires », la note minimale, « Autour de moi » (`near` 25 km, repli « saisissez une ville », puce de rayon actif) et la recherche libre `search` (nom, ville, description — `?city=` inchangé) ; la distance est filtrée en SQL pour que `total`/`totalPages` restent justes ; verrou i18n **1749 → 1762** ; preuves : `geo-distance` 8/8, `search-near-me-button` 1/1, vitest **146 fichiers / 812 tests**, sondes runtime FR/EN. **Reste** : **B9 → T-261** (préférences de notification par utilisateur : `users.notification_prefs`, `null` = héritage global). `STATE.md` est à rafraîchir en fin de session (R7 : un
