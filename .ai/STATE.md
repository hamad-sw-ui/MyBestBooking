...
- **HEAD Git** : `f0e9b6c` (dernier commit figé — **T-232 / T-233 / T-234 + volet T-240** (audit n°3) : (a) **T-232** dates de séjour et fuseaux — helper unique `src/lib/dates.ts` (date civile jamais décalée vs instant à fuseau explicite), `pg` qui rend les colonnes `date` en chaînes (`setTypeParser(1082)`), fenêtres civiles câblées (analytics, calendriers, dashboard, tunnel, mon-compte, gestionnaire de réservations), **0 `toLocaleDateString` restant** dans `src/`, `users.timezone` validé et réellement lu ; (b) **T-233** cascade de suspension d'hôte — `src/lib/host-suspension.ts` bascule les annonces `active ↔ suspended` en transaction et de façon idempotente (suspension unitaire + bulk), recherche/fiche/tunnel filtrent les hôtes inactifs, cache du catalogue ancré sur `globalThis` et invalidé à chaque changement, garde de visibilité hors cache sur la fiche ; (c) **T-234** expiration paresseuse — purge extraite dans `src/lib/booking-request-expiration.ts` (le cron n'en garde que le branchement), exécutée **dans la transaction** de `POST /api/bookings` et de `GET /api/bookings/quote`, bornée à la chambre et à la fenêtre, notifications après commit, `loadBookedCounts` qui ne compte plus une demande morte ; (d) **T-240** tests multi-fuseaux `transitionError`/`isReviewEligible` et « aujourd'hui » métier centralisé (`civilToday("UTC")`). Preuves : `npm run ci` **verte** (typecheck 0 · lint 0/0 · i18n 0 candidat, catalogue **1640** · vitest **709 tests / 120 fichiers, 0 échec** · build · smoke **95/95**) · `ai:check` **19 OK / 1 warn / 0 fail** (warn R7 documenté : un commit ne peut pas citer son propre SHA) · runtime réel (fiche 200 → 404 → 404 → 200, total d'annonces publiques 8 → 0 → 8, réservation 400 « Hébergement non disponible » puis 201, demande expirée purgée par le tunnel — **201 au lieu de 409**, le 409 du constat étant reproduit en retirant le correctif —, disponibilité 0/1 → 1/0). Rapport : `REPORTS/validation_T232_T234_2026-09-10_dates_suspension_expiration.md`. Base remise à l'état seed exact (8 users / 8 annonces `active` / 34 réservations / 25 avis), sondes et comptes de test supprimés. Tasks encore ouvertes : T-235 → T-239 et T-241 au BACKLOG. Rappel : `STATE.md` est à mettre à jour en fin de session (motif toléré R7) — ce fichier étant mis à jour par un commit, il ne peut pas citer le SHA du commit qui le contient.
