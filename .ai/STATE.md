...
- **HEAD Git** : `94866d3` — workspace courant : **T-217 + T-219 CORRIGÉS (VALIDÉS)** (2026-09-10), implémentation des **dix constats P1–P10** de l'audit runtime (`docs/analyse_2026-09-10_audit_runtime_fonctionnalites.md`) puis clôture de complétude. **P1** : fin du soft-404 (squelettes **feuilles** ; balayage exhaustif des **10 pages `notFound()`** en production → 404 partout, 14 sondes ; 18 pages valides → 200) — l'assertion smoke `/maintenance` accepte `200` (actif) ou `307` (inactif). **P2** : `/dashboard/properties/[id]` rendu côté serveur (`isUuid`, `notFound()`, garde de rôle). **P3** : section « Avis » dans `/dashboard/settings` (modération ; avis `pending` quand active). **P4/P8** : reçus des séjours passés (prouvé bout en bout : voyageur 200, autrui 403, anonyme 401) ; carte billing « Versements et relevés ». **P5** : `/dashboard/promotions/[id]` (`null` = illimité, `currentUses` préservé). **P6** : `/dashboard/rooms/[id]` → redirection vers `/calendrier#room-edit`. **P7** : fil vide visible 7 jours dans `/messages` (guide déjà présent sur le détail). **P9** : `/dashboard/audit` paginé via `GET /api/admin/audit` ; composants orphelins `@deprecated`. **P10** : `/dashboard/rooms` dans la navigation admin desktop + mobile. **T-219** : exports CSV distingués (« versements » vs « réservations »). **T-218 non retenu** (l'e-mail part déjà au premier message), **T-220** au backlog. Preuves : `npm run ci` **EXIT=0** (typecheck 0 · lint 0 · i18n 0 candidat, catalogue **1560** · vitest **652 tests** avec serveur dev · build 65 pages · smoke **96/96** · ai:check 19 OK / 0 fail) · base laissée au seed (8 users / 8 props / 33 bookings / 24 avis / compteurs 0). Rapports : `.ai/REPORTS/validation_T217_2026-09-10_correctifs_audit_runtime.md` (§ 5 addendum de clôture). Tâche précédente : **T-211 CORRIGÉ (VALIDÉ)** (2026-09-10), wrapper `site:audit:prod` pour audit runtime en serveur production. Livré : `scripts/site-audit-prod.mjs` + script npm `site:audit:prod`, séquence `next build` → `next start -H 0.0.0.0` → attente `/api/health` → `scripts/site-audit.mjs` → cleanup du serveur lancé ; port configurable `SITE_AUDIT_PROD_PORT`/`--port`, défaut 3100 avec recherche de port libre, option `--skip-build`. Preuves : 🔨 `node --check scripts/site-audit-prod.mjs` OK · ▶️ `npm run site:audit:prod` build 65 pages + crawl 247 pages / 0 issue + cleanup OK · lint 0 · typecheck 0 · i18n 0 · 🧪 npm test 577 pass / 17 skip · ✅ ai:check 20 OK / 0 warn / 0 fail · git diff --check OK · aucun serveur Next persistant. Tâche précédente : **T-210 CORRIGÉ (VALIDÉ)** (2026-09-10), audit runtime complémentaire + filtre d'accueil destination seule : suppression de `checkIn`, `checkOut`, `guests` et `home-guests` du formulaire hero `/`, conservation de `action="/recherche"` + `city`, `/recherche` et fiche/réservation inchangées pour dates/voyageurs/disponibilité. Base précédente : **T-207 CORRIGÉ (VALIDÉ)** (2026-09-10), réservations sans paiement plateforme : aucun paiement voyageur plateforme, pas de carte/Stripe public, pas de CTA « Payer maintenant ». Historique précédent : **T-204 IMPLEMENTÉ (VALIDÉ)** (2026-09-07) :
