...
- **HEAD Git** : `8439584` (dernier commit figé — **MISE EN ŒUVRE DES REMARQUES D'AUDIT (LIVRÉE ET VALIDÉE)** (2026-09-10) : les **14 constats** des deux dernières campagnes sont implémentés, testés et vérifiés au runtime — audit n°2 **A1 → A11** (échéance des demandes ; séjours échus non réglés ; interrupteurs d'e-mails ; parrainage réglable ; notifications d'avis ; édition complète de chambre ; horaires d'arrivée/départ + fuseau validés ; labels/badges réservés à l'admin en PUT et POST ; libellé du fil par acteur ; `users.suspended_at` distinct de `deleted_at` avec refus 409 de réactivation d'un compte anonymisé ; codes de secours 2FA hachés à usage unique + reset support `user.2fa.reset`) et audit n°4 **T-242 → T-244** (anonymisation transactionnelle complète ; purge technique en cron ; « Reste vendable » au calendrier hôte). Migration additive `drizzle/0021_user_suspension_and_backup_codes.sql`. Preuves : `npm run ci` **verte** (typecheck 0 · lint 0 · i18n 0 candidat, catalogue **1640** · vitest **691 tests / 117 fichiers** · build · smoke **95/95**) · `ai:check` **19 OK / 1 warn / 0 fail** (warn R7 documenté : un commit ne peut pas citer son propre SHA) · runtime réel (parcours 2FA complet avec refus du code réutilisé, suspension/réactivation/409, horaires et fuseau persistés, labels 403 hôte / 200 admin, calendrier « reste 2 (1 réservé) »). Rapports : `REPORTS/analyse_impact_T-221_2026-09-10_mise_en_oeuvre_audits.md`, `REPORTS/analyse_conception_T-221_2026-09-10_mise_en_oeuvre_audits.md` et `REPORTS/validation_T221_T231_2026-09-10_mise_en_oeuvre_audits.md`. Base remise à l'état seed exact (8 users / 8 properties / 31 bookings / 22 avis), sondes et comptes de test supprimés. Analyses sources de la même journée (inchangées) : audit n°4 (`docs/analyse_2026-09-10_audit_runtime_profondeur.md`, 3 constats N1→N3), audit n°3 (`docs/analyse_2026-09-10_audit_runtime_scenarios.md`, 13 constats F1→F13 → T-232 → T-241), audit n°2 (`docs/analyse_2026-09-10_audit_runtime_inacheves.md`, 11 constats A1→A11 + 6 observations). Rappel : `STATE.md` est à mettre à jour en fin de session (motif toléré R7) — ce fichier étant mis à jour par un commit, il ne peut pas citer le SHA du commit qui le contient.
