...
- **HEAD Git** : `221884b` — workspace courant : **T-217 CORRIGÉ (VALIDÉ)** (2026-09-10), implémentation des **dix constats P1–P10** de l'audit runtime (`docs/analyse_2026-09-10_audit_runtime_fonctionnalites.md`, commit `e8ecc71`). **P1** : fin du soft-404 — `src/app/loading.tsx` racine remplacé par des squelettes **feuilles** (`src/components/page-loading.tsx` + 11 routes de liste sans enfant dynamique, jamais au-dessus d'une route `[id]`) ; matrice production **8 URL invalides → 404**, 18 pages valides → 200 ; l'assertion smoke `/maintenance` accepte désormais `200` (actif) ou `307` (inactif). **P2** : `/dashboard/properties/[id]` rendu côté serveur (`isUuid`, `notFound()`, garde de rôle) + client amorcé par props. **P3** : section « Avis » dans `/dashboard/settings` (bascule `reviews.requireModeration` ; avis `pending` quand active). **P4/P8** : reçus des séjours passés (`BookingRowActions`) ; carte billing « Versements et relevés » + pointeur vers les reçus. **P5** : `/dashboard/promotions/[id]` + `PromotionEditForm` (PATCH existant, `null` = illimité, `currentUses` préservé). **P6** : `/dashboard/rooms/[id]` → redirection vers `/calendrier#room-edit`. **P7** : fil vide visible 7 jours dans `/messages` (compteurs T-206/F9 conservés). **P9** : `/dashboard/audit` paginé via `GET /api/admin/audit` ; composants orphelins `@deprecated` documentés. **P10** : `/dashboard/rooms` dans la navigation admin desktop + mobile. Preuves : 🔨 typecheck 0 · lint 0 · 🔍 i18n:check 0 (catalogue **1559**) · 🧪 vitest **109 fichiers / 652 tests, 0 échec** (+10) · 🔨 build 65 pages · ▶️ smoke **96/96** · ▶️ matrice prod 8×404 / 18×200 · ▶️ runtime ciblé P2–P10 · ✅ ai:check 19 OK / 0 fail · base laissée au seed (8 users / 8 props / 33 bookings / 24 avis / compteurs 0). Rapport : `.ai/REPORTS/validation_T217_2026-09-10_correctifs_audit_runtime.md`. Tâche précédente : **T-211 CORRIGÉ (VALIDÉ)** (2026-09-10), wrapper `site:audit:prod` pour audit runtime en serveur production. Livré : `scripts/site-audit-prod.mjs` + script npm `site:audit:prod`, séquence `next build` → `next start -H 0.0.0.0` → attente `/api/health` → `scripts/site-audit.mjs` → cleanup du serveur lancé ; port configurable `SITE_AUDIT_PROD_PORT`/`--port`, défaut 3100 avec recherche de port libre, option `--skip-build`. Preuves : 🔨 `node --check scripts/site-audit-prod.mjs` OK · ▶️ `npm run site:audit:prod` build 65 pages + crawl 247 pages / 0 issue + cleanup OK · lint 0 · typecheck 0 · i18n 0 · 🧪 npm test 577 pass / 17 skip · ✅ ai:check 20 OK / 0 warn / 0 fail · git diff --check OK · aucun serveur Next persistant. Tâche précédente : **T-210 CORRIGÉ (VALIDÉ)** (2026-09-10), audit runtime complémentaire + filtre d'accueil destination seule : suppression de `checkIn`, `checkOut`, `guests` et `home-guests` du formulaire hero `/`, conservation de `action="/recherche"` + `city`, `/recherche` et fiche/réservation inchangées pour dates/voyageurs/disponibilité. Base précédente : **T-207 CORRIGÉ (VALIDÉ)** (2026-09-10), réservations sans paiement plateforme : aucun paiement voyageur plateforme, pas de carte/Stripe public, pas de CTA « Payer maintenant ». Historique précédent : **T-204 IMPLEMENTÉ (VALIDÉ)** (2026-09-07) :
