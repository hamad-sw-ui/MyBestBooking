...
- **HEAD Git** : `9158ff5` — workspace courant : **T-215 + T-216 CORRIGÉS (VALIDÉS)** (2026-09-10), implémentation sans régression des plans §1.5 et §3.3 de l'analyse produit. **T-215** : commission d'un hôte éditable par l'admin **pour tout statut d'approbation** — `GET /api/admin/hosts/[id]` (aperçu lecture seule : taux hôte/global/effectif, héritage, explicites, détail par hébergement), `PATCH … action:"updateCommission"` (`commissionRate` 0–100 ou `null` = héritage ; `applyTo: inherited|listed` + `propertyIds` ≤ 100 ; propagation **jamais implicite**, snapshots de vente intacts), `HostCommissionEditor` sur `/dashboard/users` (une seule zone de saisie par hôte, impact « Héritent : N · Taux explicite : M », case de propagation décochée par défaut), correction **BUG-050** (`propertyCount` toujours nul dans la liste des hôtes). **T-216** : gestion manuelle du statut d'une réservation **dans la liste** `/dashboard/bookings` — `availableTransitions()` (pure, dérivée de `transitionError()` + garde paiement), `BookingStatusSelect` (badge historique conservé sinon), audit `booking.status.update` sur les transitions **et** l'annulation ; `PUT /api/bookings/[id]` reste l'unique source de vérité (FSM, verrou, e-mails, fidélité). Preuves : 🔨 typecheck 0 · lint 0 · 🔍 i18n:check 0 (catalogue **1532**) · 🔨 build 65 pages · 🧪 vitest **642 / 107 fichiers, 0 échec** · ▶️ runtime API (propertyCount 8, propagation listed/inherited/reset, 400/403/404, audits ; `pending→confirmed` 200, clôtures 400/409/200, terminal 400) · ▶️ SSR `/dashboard/users` + `/dashboard/bookings` · ▶️ `npm run ci` verte (smoke **95/95**) · ✅ ai:check 19 OK / 0 fail · base laissée à l'état seed (8 users / 8 props / 33 bookings / 24 avis / outbox 0). Tâche précédente : **T-211 CORRIGÉ (VALIDÉ)** (2026-09-10), wrapper `site:audit:prod` pour audit runtime en serveur production. Livré : `scripts/site-audit-prod.mjs` + script npm `site:audit:prod`, séquence `next build` → `next start -H 0.0.0.0` → attente `/api/health` → `scripts/site-audit.mjs` → cleanup du serveur lancé ; port configurable `SITE_AUDIT_PROD_PORT`/`--port`, défaut 3100 avec recherche de port libre, option `--skip-build`. Preuves : 🔨 `node --check scripts/site-audit-prod.mjs` OK · ▶️ `npm run site:audit:prod` build 65 pages + crawl 247 pages / 0 issue + cleanup OK · lint 0 · typecheck 0 · i18n 0 · 🧪 npm test 577 pass / 17 skip · ✅ ai:check 20 OK / 0 warn / 0 fail · git diff --check OK · aucun serveur Next persistant. Tâche précédente : **T-210 CORRIGÉ (VALIDÉ)** (2026-09-10), audit runtime complémentaire + filtre d'accueil destination seule : suppression de `checkIn`, `checkOut`, `guests` et `home-guests` du formulaire hero `/`, conservation de `action="/recherche"` + `city`, `/recherche` et fiche/réservation inchangées pour dates/voyageurs/disponibilité. Base précédente : **T-207 CORRIGÉ (VALIDÉ)** (2026-09-10), réservations sans paiement plateforme : aucun paiement voyageur plateforme, pas de carte/Stripe public, pas de CTA « Payer maintenant ». Historique précédent : **T-204 IMPLEMENTÉ (VALIDÉ)** (2026-09-07) :
