...
- **HEAD Git** : `a560e42` — workspace courant : **AUDIT RUNTIME N°4 (PROFONDEUR, ANALYSE SEULE)** (2026-09-10), `docs/analyse_2026-09-10_audit_runtime_profondeur.md` (copie `.ai/REPORTS/analyse_runtime_n4_2026-09-10_profondeur.md`) : **3 constats nouveaux tracés T-242 → T-244** (anonymisation de compte limitée à `users` — l'identité survit dans `bookings.guest_*`, `email_outbox.to` et `audit_log.targetEmail` ; aucune purge des sessions expirées, e-mails livrés et journaux d'audit ; `GET /api/rooms/[id]/availability` renvoie le stock déclaré sans retirer les séjours → « 2 disponibles » affichés pour 1 réel) **+ 7 confirmations chiffrées** des constats F1/F2/F3/F4/F5/F9/F12 (dates et fuseaux, suspension d'hôte, expiration paresseuse, quota compté avant validation, heure d'arrivée, fuseau décoratif, analytics figé) et un inventaire « vérifié sain » (matrice de permissions 24 cas × 5 identités / 120 requêtes, révocation de session à la suspension-suppression-changement de mot de passe, restitution unique promo/wallet, idempotence `email_outbox`, `POST /api/seed` fermé hors environnement démo). Base laissée au seed exact (8 users / 8 props / 33 bookings / 24 avis / compteurs 0) ; sondes supprimées ; **chantier T-221 → T-231 en cours dans l'arbre** (A1 → A6 livrés localement : échéance des demandes, règlements échus, interrupteurs d'e-mails, parrainage, notifications d'avis, édition de chambre ; A7 → A11 restants). Analyse précédente (même journée) : **AUDIT RUNTIME N°3 (SCÉNARIOS, ANALYSE SEULE)** (2026-09-10), `docs/analyse_2026-09-10_audit_runtime_scenarios.md` (copie `.ai/REPORTS/analyse_runtime_n3_2026-09-10_scenarios.md`) : **13 constats F1→F13** (dates de séjour décalables d'un jour selon le fuseau navigateur/serveur et réglages `timezone` décoratifs ; hôte suspendu dont les annonces restent actives et réservables ; demande en attente bloquant les dates sans expiration paresseuse ; quota de réservation 10/h compté avant validation ; heure d'arrivée estimée jamais restituée ; validation/rejet d'annonce sans notification ni motif lisible ; wishlist partagée indexable et sans expiration ; désabonnement promis par la page Confidentialité mais inexistant ; états « supprimé »/« suspendu » confondus ; analytics sans période ni export ; erreurs d'API partielles et champs inconnus acceptés en 200), tracés dans `BACKLOG.md` sous **T-232 → T-241**. Méthode : 43 pages et 67 routes API, **124 liens internes suivis (0 cassé)**, analyse statique de tous les `<Button>` (**0 mort**), sondes HTTP/PostgreSQL, reproductions multi-fuseaux (`UTC`/`Africa/Douala`/`America/Los_Angeles`/`Pacific/Kiritimati`) et cron réel exécuté ; base laissée au seed exact (8 users / 8 props / 33 bookings / 24 avis / compteurs 0 / 1 `app_settings`). Aucun code produit modifié par l'analyse ; un chantier d'implémentation T-221/T-222 reste en cours dans l'arbre de travail, hors commit d'analyse. Analyse précédente (même journée) : **AUDIT RUNTIME N°2**, `docs/analyse_2026-09-10_audit_runtime_inacheves.md` : **11 constats A1→A11** + 6 observations, tracés dans `BACKLOG.md` sous **T-221 → T-231** (T-221 échéance invisible des demandes de réservation ; T-222 séjours échus non réglés sans vue ni relance ; T-223 interrupteurs `notifications` sans UI ; T-224 parrainage non réglable ; T-225 avis sans e-mail ; T-226 édition de chambre incomplète ; T-227 horaires d'arrivée/départ inéditables ; T-228 labels/badges non administrables (badge « Éco » inatteignable) ; T-229 fil de conversation du back-office (libellé faux / admin 403) ; T-230 suspension ≡ suppression (`deleted_at`) ; T-231 2FA sans codes de secours ni reset support). Méthode : 43 pages et 67 routes API recensées, **261 liens internes suivis (0 cassé)**, `npm run site:audit:prod` **262 pages / 0 issue**, sondes d'exécution (échéance de demande, expiration sans e-mail, fil par rôle, suspension/réactivation, suppression de compte puis réactivation admin, constat de paiement), base laissée au seed (8 users / 8 props / 33 bookings / 24 avis / compteurs 0). Aucun code produit modifié. Référence livrée inchangée : **T-217 + T-219 CORRIGÉS (VALIDÉS)** — P1 soft-404 (squelettes feuilles, vrais 404), P2 hébergement RSC, P3 modération des avis, P4 reçus des séjours passés (preuve e2e 200/403/401), P5 édition des promotions, P6 redirection chambres, P7 fenêtre 7 j des fils vides, P8 « Versements et relevés », P9 audit paginé, P10 chambres dans la navigation, T-219 exports CSV distincts, T-218 non retenu, T-220 au backlog. Preuves T-217/T-219 : `npm run ci` **EXIT=0** (typecheck 0 · lint 0 · i18n 0 candidat, catalogue **1560** · vitest **652 tests** · build 65 pages · smoke **96/96**) · `ai:check` **20 OK / 0 warn / 0 fail** après `docs(state)`. Rapport : `.ai/REPORTS/validation_T217_2026-09-10_correctifs_audit_runtime.md`. Tâche précédente : **T-211 CORRIGÉ (VALIDÉ)** (2026-09-10), wrapper `site:audit:prod` pour audit runtime en serveur production. Livré : `scripts/site-audit-prod.mjs` + script npm `site:audit:prod`, séquence `next build` → `next start -H 0.0.0.0` → attente `/api/health` → `scripts/site-audit.mjs` → cleanup du serveur lancé ; port configurable `SITE_AUDIT_PROD_PORT`/`--port`, défaut 3100 avec recherche de port libre, option `--skip-build`. Preuves : 🔨 `node --check scripts/site-audit-prod.mjs` OK · ▶️ `npm run site:audit:prod` build 65 pages + crawl 247 pages / 0 issue + cleanup OK · lint 0 · typecheck 0 · i18n 0 · 🧪 npm test 577 pass / 17 skip · ✅ ai:check 20 OK / 0 warn / 0 fail · git diff --check OK · aucun serveur Next persistant. Tâche précédente : **T-210 CORRIGÉ (VALIDÉ)** (2026-09-10), audit runtime complémentaire + filtre d'accueil destination seule : suppression de `checkIn`, `checkOut`, `guests` et `home-guests` du formulaire hero `/`, conservation de `action="/recherche"` + `city`, `/recherche` et fiche/réservation inchangées pour dates/voyageurs/disponibilité. Base précédente : **T-207 CORRIGÉ (VALIDÉ)** (2026-09-10), réservations sans paiement plateforme : aucun paiement voyageur plateforme, pas de carte/Stripe public, pas de CTA « Payer maintenant ». Historique précédent : **T-204 IMPLEMENTÉ (VALIDÉ)** (2026-09-07) :
