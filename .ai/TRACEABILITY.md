...
| T-011 | Framework v1.1.0 : élargissement à la complétude produit (ADR-006) — FEATURES.md, PRODUCT_ACCEPTANCE.md, R14-R17, tag 🎯 PROMISED, Playwright installé, BACKLOG.md réécrit | C (§15.0-bis évolution règles §1-§22) | CORRIGÉ (INSPECTION) | 🔍 20 documents obligatoires (2 nouveaux : FEATURES, PRODUCT_ACCEPTANCE) · 🔍 script check-ai passe de 13 à 17 règles (R14-R17 ajoutées) · 🔍 CODING_RULES §16 gagne le tag 🎯 PROMISED · 🔨 typecheck OK · 🧪 43/43 tests (aucune régression) · ▶️ `npm run ai:check` → 13 OK · 4 warn · 0 fail — les warns R14/R15 révèlent 5 tables sans endpoint + 2 boutons UI orphelins (nouvelle roadmap T-015/T-018 tracée dans BACKLOG) · 🔍 Playwright installé (dev-dep, Chromium à installer côté CI/local car sandbox n'a pas d'accès CDN Google) · 🔍 6 smoke tests E2E documentés dans `tests/e2e/smoke.spec.ts` | commit T-011 | `ADR-006` · 3 rapports impact/conception/débat |
