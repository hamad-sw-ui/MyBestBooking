...
| T-001 | JWT_SECRET obligatoire au boot (corrige BUG-001) | C | CORRIGÉ (VALIDÉ) | 🔍 fallback `"mybestbooking-secret-key-2025"` retiré de `src/lib/auth.ts` · 🔨 `npm run typecheck` OK · 🔨 `npm run build` OK avec JWT_SECRET · 🧪 `npm test` : 26 tests passent (17 utils + 9 auth §13.5), dont un test qui vérifie qu'un JWT signé avec un autre secret n'est pas vérifiable (contrat de sécurité) · ▶️ `POST /api/auth/login` retourne 200 avec le compte admin post-changement (non-régression) · ▶️ `npm run ai:check` retourne 12 OK · 2 warn · 0 fail | commit T-001 | `REPORTS/analyse_impact_2026-08-20_jwt_secret.md` · `REPORTS/analyse_conception_2026-08-20_jwt_secret.md` · `REPORTS/debat_technique_2026-08-20_jwt_secret.md` · `ADR/ADR-003_JWT_Secret_Obligatoire.md` |
