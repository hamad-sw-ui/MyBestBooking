...
| T-003 | Proxy d'auth (BUG-005) : redirige /mon-compte, /mes-reservations, /mes-favoris, /messages, /reservation, /dashboard/* vers /connexion?next= si non authentifié | S | CORRIGÉ (VALIDÉ) | 🔍 `src/proxy.ts` (Next 16 renomme middleware.ts) · 🔨 typecheck OK · 🧪 38 tests passent (17 utils + 9 auth + 7 seed + 5 proxy) · ▶️ curl sans cookie sur les 6 routes protégées → 307 vers /connexion avec next= · ▶️ curl avec cookie login admin → 200 · ▶️ /  et /recherche restent 200 (non-régression publique) | commit T-003 | `REPORTS/..._middleware_auth.md` · `ADR/ADR-005_Middleware_Auth.md` |
