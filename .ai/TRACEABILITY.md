...
| T-012 | Disponibilité + chevauchement bookings (S) | S | CORRIGÉ (VALIDÉ) | 🔍 `src/lib/availability.ts` (hasOverlap pur) · 🔍 index `idx_bookings_room_dates` (migration 0002_brief_cannonball) · 🔍 transaction Drizzle avec SELECT FOR UPDATE dans POST /api/bookings · 🔨 typecheck OK · 🧪 **54/54 tests** (17 utils + 9 auth + 7 seed + 5 proxy + 5 rate-limit + 7 availability + 4 bookings intégration) · ▶️ curl manuel : chambre qty=2 accepte 2 bookings chevauchants puis refuse le 3e avec HTTP 409 « Cette chambre n'est plus disponible pour ces dates » · ▶️ adjacent Feb 5=checkOut / Feb 5=checkIn accepté (convention hôtelière) · ▶️ disjoint accepté | commit T-012 | `REPORTS/analyse_*_booking_availability.md` |
