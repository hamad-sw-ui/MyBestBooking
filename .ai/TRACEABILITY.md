...
| T-015 | 6 endpoints mutations manquants (S) | S | CORRIGÉ (VALIDÉ) | 🔍 `POST /api/conversations` + `GET` + idempotent · 🔍 `GET/POST /api/messages` (participant check, unread counters, senderType user/host) · 🔍 `POST /api/reviews/[id]/reply` (host owner check) · 🔍 `POST /api/properties/[id]/validate` (admin, approve/reject/suspend + validatedAt/By) · 🔍 `GET /api/wishlists/shared/[token]` (public, expose seulement name+items) · 🔍 `GET/POST /api/promotions` + `PATCH/DELETE /api/promotions/[id]` (admin) · 🔨 typecheck OK · 🧪 71/71 tests · ▶️ E2E curl : promotion SUMMER26 créée · property suspend→approve OK · conversation créée · message envoyé et lu · wishlist share token OK, token invalide → 404 · ▶️ ai:check R14 : 5 tables sans endpoint → 3 restantes (rate_plans, room_availability, wishlist_items — les 2 premières T-018) | commit T-014-T-015 | `REPORTS/analyse_impact_*_mutations_manquantes.md` |
